...
public class MatrixUtils {
    public Matrix4f createTransformation(Transform transform) {
        ...
        transformationMatrix = transformationMatrix.multiply( // Position
            Matrix4f.translate(
                transform.getPos().x, transform.getPos().y, transform.getPos().z
            )
        );
        transformationMatrix = transformationMatrix.multiply( // Rotate X
            Matrix4f.rotate(transform.getRot().x, 1 ,0 ,0)
        );
        transformationMatrix = transformationMatrix.multiply( // Rotate Y
            Matrix4f.rotate(transform.getRot().y, 0 ,1 ,0)
        );
        transformationMatrix = transformationMatrix.multiply( // Rotate Z
            Matrix4f.rotate(transform.getRot().z, 0 ,0 ,1)
        );
        transformationMatrix = transformationMatrix.multiply( // Scale
            Matrix4f.scale(
    ...
    public Matrix4f createCameraMatrix(Camera camera) {
        ...
        Vector3f position = camera.getPos();
    ...
    public Matrix4f createProjectionMatrix(float FOV, float NEAR_PLANE, float FAR_PLANE) {
        ...
        float aspectRatio = (float)  1280 / (float) 720; //TODO window size
